...
public class PostServiceImpl implements PostService {
    ...
    public Post addPost(Post post) throws Exception {
        try{
            ...
            throw new Exception("could not save the post" + e);
        }
        return post;
    }
    ...
    public List<Post> getPosts() {
        ...
        List<Post> posts = postEntities.stream().map((postEntity)->Post.builder().id(postEntity.getId()).timeStamp(postEntity.getTimeStamp()).email(postEntity.getEmail()).name(postEntity.getName()).post(postEntity.getPost()).image(postEntity.getImage()).profilePic(postEntity.getProfilePic()).
